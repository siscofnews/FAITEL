...
interface PoleLogoUploadProps {
  poleId: string;
  currentLogoUrl?: string | null;
  onUploadSuccess: (url: string) => void;
}

export function PoleLogoUpload({ poleId, currentLogoUrl, onUploadSuccess }: PoleLogoUploadProps) {
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const fileName = `polo/${poleId}/logo-${Date.now()}.${ext}`;
      ...
      const { error: updateError } = await supabase.from('college_polo' as any).update({ logo_url: publicUrl }).eq('id', poleId);
      if (updateError) throw updateError;
      setPreview(publicUrl);
      onUploadSuccess(publicUrl);
      toast({ title: "Logo do Polo enviada" });
    } catch (error:any) {
  ...
  const handleRemove = async () => {
    try { const { error } = await supabase.from('college_polo' as any).update({ logo_url: null }).eq('id', poleId); if (error) throw error; setPreview(null); onUploadSuccess(''); toast({ title: "Logo removida" }); } catch (error:any) { toast({ title: "Erro ao remover logo", description: String(error?.message||''), variant: "destructive" }); }
  };
  ...
  return (
    <div className="space-y-2">
      ...
      <div>
        <input type="file" id={`polo-logo-${poleId}`} accept="image/png,image/jpeg,image/jpg" onChange={handleFileChange} disabled={isUploading} className="hidden" />
        <Label htmlFor={`polo-logo-${poleId}`}>
