      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <div className="space-y-4">
            {rows.map(r => (
              <div key={r.id} className="border p-4 rounded space-y-4">
                {/* Info Section */}
                <div className="space-y-2">
                  <Label className="text-sm font-semibold">Nome da Faculdade</Label>
                  <Input value={r.name || ''} onChange={(e) => update(r.id, { name: e.target.value })} placeholder="Nome da faculdade" />
                </div>

                {/* Media Section */}
                <div className="space-y-3">
                  <div className="text-sm font-semibold text-primary">Mídia e Identidade Visual</div>
                  <div className="grid md:grid-cols-3 gap-4 border-l-2 border-primary/20 pl-4">
                ...
                <div className="space-y-3">
                  <div className="text-sm font-semibold text-primary">Financeiro e Estatísticas</div>
                  <div className="grid md:grid-cols-2 gap-4 border-l-2 border-primary/20 pl-4">
                    <div className="space-y-2">
                      <Label className="text-xs">Vínculo Financeiro</Label>
                      <select className="w-full border rounded px-3 py-2 text-sm" value={r.finance_faculty_id || ''} onChange={(e) => update(r.id, { finance_faculty_id: e.target.value })}>
                        <option value="">Selecione uma faculdade</option>
                        {faculties.map((f: any) => (<option key={f.id} value={f.id}>{f.name || f.id}</option>))}
                      </select>
                    </div>
                    <div className="space-y-2">
                      <Label className="text-xs">Hierarquia</Label>
                      <div className="text-sm bg-muted/50 px-3 py-2 rounded">
                        Polos: <strong>{counts[r.id] || 0}</strong> • Núcleos: <strong>{deep[r.id] || 0}</strong>
                      </div>
                    </div>
                  </div>
                  {fin[r.id] && (
                    <div className="grid md:grid-cols-3 gap-3 text-xs border-l-2 border-primary/20 pl-4">
                      <div className="bg-green-50 dark:bg-green-950 px-3 py-2 rounded">
                        <div className="text-muted-foreground">Entradas</div>
                        <div className="font-semibold text-green-700 dark:text-green-400">{fin[r.id].entries.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</div>
                      </div>
                      <div className="bg-red-50 dark:bg-red-950 px-3 py-2 rounded">
                        <div className="text-muted-foreground">Despesas</div>
                        <div className="font-semibold text-red-700 dark:text-red-400">{fin[r.id].expenses.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</div>
                      </div>
                      <div className="bg-blue-50 dark:bg-blue-950 px-3 py-2 rounded">
                        <div className="text-muted-foreground">Saldo</div>
                        <div className="font-semibold text-blue-700 dark:text-blue-400">{fin[r.id].balance.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</div>
                      </div>
                    </div>
                  )}
                  {!fin[r.id] && <div className="text-xs text-muted-foreground border-l-2 border-primary/20 pl-4">Sem vínculo financeiro configurado</div>}
                </div>

                {/* Actions Section */}
                <div className="flex flex-wrap gap-2 pt-2 border-t">
                  <Button variant="default" onClick={() => window.location.href = `/tesouraria/consolidacao?entity=FACULDADE&rootId=${r.id}`}>Ver Consolidação</Button>
                  <Button variant="secondary" onClick={() => window.location.href = '/tesouraria'}>Tesouraria EAD</Button>
                  <Button variant="outline" onClick={() => { if ((counts[r.id] || 0) > 0) return alert('Exclua os polos antes.'); remove(r.id); }}>Excluir Matriz</Button>
                </div>
              </div>
            ))}
            {rows.length > 0 && (
