...
export default function CostCenters() {
  ...
  const loadRows = async () => { if (!entityId) return; const { data } = await supabase.from('cost_centers').select('*').eq('entity', entity).eq('entity_id', entityId).order('code'); setRows(data||[]); };
  useEffect(()=>{ loadEntities(); setEntityId(''); }, [entity]);
  useEffect(()=>{ loadRows(); }, [entityId]);
  const add = async () => { if (!entityId || !form.code || !form.name) return; await supabase.from('cost_centers').insert({ entity, entity_id: entityId, code: form.code, name: form.name }); setForm({ code:'', name:'' }); loadRows(); };
  const update = async (id: string, patch: any) => { await supabase.from('cost_centers').update(patch).eq('id', id); loadRows(); };
  const remove = async (id: string) => { await supabase.from('cost_centers').delete().eq('id', id); loadRows(); };
  return (
    <MainLayout>
      <Card>
