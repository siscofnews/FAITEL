...
const ROLES = ['obreiro','pastor','pastora','diacono','diaconisa','missionario','missionaria','evangelista','presbitero','bispo','bispa','apostolo','apostola'];

export default function CadastroMinisterial() {
  const { toast } = useToast();
  const [params] = useSearchParams();
  const [churches, setChurches] = useState<any[]>([]);
  const [form, setForm] = useState<any>({ full_name: "", birth_date: "", email: "", phone: "", church_id: "", role: "", photo_path: "" });
  ...
  const onUpload = async (file: File | null) => {
    ...
    try {
      ...
      const name = `${form.church_id || 'public'}/minister-${Date.now()}.${ext}`;
  ...
  const save = async () => {
    try {
      if (!form.full_name || !form.birth_date || !form.church_id || !form.role) { toast({ title: 'Preencha campos obrigatórios', variant: 'destructive' }); return; }
      const { data: m, error } = await supabase
        .from('members')
        .insert({ full_name: form.full_name, birth_date: form.birth_date, email: form.email, phone: form.phone, church_id: form.church_id, role: form.role, is_active: false })
        .select('id')
        .single();
      if (error) throw error;
      ...
      toast({ title: 'Cadastro ministerial enviado' });
    } catch (e:any) { toast({ title: 'Erro', description: e.message, variant: 'destructive' }); }
  };
  ...
  return (
    <div className="container mx-auto max-w-2xl py-6">
      <Card>
        <CardHeader><CardTitle>Cadastro Ministerial</CardTitle></CardHeader>
        <CardContent className="space-y-3">
          ...
          <Select value={form.role} onValueChange={(v)=>setForm((p:any)=>({ ...p, role: v }))}>
            <SelectTrigger><SelectValue placeholder="Cargo Ministerial" /></SelectTrigger>
            <SelectContent>
